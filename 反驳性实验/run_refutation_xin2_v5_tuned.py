"""
run_refutation_xin2_v5_tuned.py
=================================
XIN_2 因果推断反驳实验 v5 调优版 ——「LSTM-VAE 联合训练微创新」架构
...
用法：
  # 稳定性诊断
  python run_refutation_xin2_v5_tuned.py --mode stability --sample_size 2000 --n_bootstrap 3

  # 消融实验（核心新增实验）
  python run_refutation_xin2_v5_tuned.py --mode ablation --sample_size 3000 --ablation_n_ops 5

  # 全部反驳实验
  python run_refutation_xin2_v5_tuned.py --mode all --workers 4

  # 关闭某个微创新进行对比
  python run_refutation_xin2_v5_tuned.py --mode stability --no_dual_stream
  python run_refutation_xin2_v5_tuned.py --mode stability --no_curriculum
  python run_refutation_xin2_v5_tuned.py --mode stability --no_grad_proj
  python run_refutation_xin2_v5_tuned.py --mode stability --no_uncertainty_weight
"""
...
PATIENCE                   = 6
MIN_TRAIN_SIZE             = 100
MIN_VALID_RESIDUALS        = 36
F_STAT_THRESHOLD           = 5.0   # 从原始 10 放宽到 5，尽量保留边界弱工具做探索性比较
MIN_BOOTSTRAP_SUCCESS_RATE = 0.40
SAFE_X_MAX_COUNT           = 28    # v5 更易受高维控制集影响，因此比 RF 版更激进地裁剪
...
# ─── v5 微创新超参 ───────────────────────────────────────────────
# 微创新 B：双流潜变量维度
LATENT_DIM_CAUSAL = 12    # z_causal 维度（更小，降低小样本过拟合）
LATENT_DIM_RECON  = 24    # z_recon 维度（保留重建能力，减少冗余自由度）
LAMBDA_ORTH       = 0.005 # 正交性损失权重

# 微创新 C：课程式训练阶段比例和总轮数
MAX_EPOCHS_JOINT   = 60   # 联合训练总轮数（三阶段共用）
PHASE1_RATIO       = 0.15 # 预热期占比（只训练 VAE）
PHASE2_RATIO       = 0.25 # 过渡期占比（逐步引入因果）
# 剩余 60% 为精调期（因果为主，重建降权）
LAMBDA_RECON_FINAL = 0.4  # 精调期重建损失权重（< 1，降权）
...
def refine_safe_x(op: str, df: pd.DataFrame, safe_x: list,
    ...
    for var in safe_x:
        ...
        scored.append((corr_y + 0.5 * corr_d, corr_y, corr_d, var))
...
def train_one_op(op: str, df: pd.DataFrame, safe_x: list,
    ...
    # ── Bootstrap 聚合 ──────────────────────────────────────────────
    min_success = max(1, int(np.ceil(n_bootstrap * MIN_BOOTSTRAP_SUCCESS_RATE)))
...
def run_stability_diagnosis(df, ops, states, workers=4, dag_roles: dict = None,
    ...
    print(f" 调优阈值：F≥{F_STAT_THRESHOLD}, 残差≥{MIN_VALID_RESIDUALS}, 成功率≥{MIN_BOOTSTRAP_SUCCESS_RATE:.0%}, safe_x≤{SAFE_X_MAX_COUNT or '∞'}")
    ...
    out_path = os.path.join(STABILITY_OUT_DIR, "stability_diagnosis_v5_tuned.csv")
...
def run_placebo(df, ops, states, n_permutations=5, workers=4,
    ...
    if innov_cfg is None:
        innov_cfg = {}
    ckpt_path   = os.path.join(PLACEBO_OUT_DIR, "checkpoint_placebo_v5_tuned.jsonl")
    states_list = list(states)
    tasks = []
    for op in sorted(ops):
        if df[op].std() < 0.1:
    ...
    out_path = os.path.join(PLACEBO_OUT_DIR, "refutation_placebo_v5_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
...
def run_random_confounder(df, ops, states, n_confounders=5, n_repeats=1,
    ...
    if innov_cfg is None:
        innov_cfg = {}
    ckpt_path   = os.path.join(RANDOM_CONFOUNDER_OUT_DIR, "checkpoint_rc_v5_tuned.jsonl")
    states_list = list(states)
    print("[预计算原始 θ ...]")
    orig_thetas = {}
    for op in sorted(ops):
    ...
    out_path = os.path.join(RANDOM_CONFOUNDER_OUT_DIR, "refutation_rc_v5_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"结果已保存：{out_path}"); return df_out
...
def run_data_subset(df, ops, states, n_subsets=8, subset_frac=0.8,
    ...
    if innov_cfg is None:
        innov_cfg = {}
    ckpt_path   = os.path.join(DATA_SUBSET_OUT_DIR, "checkpoint_ds_v5_tuned.jsonl")
    states_list = list(states)
    ...
    out_path = os.path.join(DATA_SUBSET_OUT_DIR, "refutation_ds_v5_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"结果已保存：{out_path}"); return df_out
...
def run_ablation(df, ops, states, dag_roles: dict = None, n_ops: int = 5):
    ...
    out_path = os.path.join(ABLATION_OUT_DIR, "ablation_study_v5_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"\n结果已保存：{out_path}")
    return df_out
...
def parse_args():
    p = argparse.ArgumentParser(
        description="XIN_2 因果推断反驳实验 v5 调优版（LSTM-VAE 联合训练 + 微创新 A/B/C/D）",
...
建议执行顺序：
  ...
  # 5. 配合 v4 的交叉拟合策略改进
  python run_refutation_xin2_v5_tuned.py --mode stability \\
    --fold_jitter_ratio 0.1 --stratified

v5 调优版输出文件带 _v5_tuned 后缀，不覆盖原始 v5 结果。
        """,
    )
    ...
    p.add_argument("--f_stat_threshold", type=float, default=F_STAT_THRESHOLD)
    p.add_argument("--min_valid_residuals", type=int, default=MIN_VALID_RESIDUALS)
    p.add_argument("--min_success_rate", type=float, default=MIN_BOOTSTRAP_SUCCESS_RATE)
    p.add_argument("--max_safe_x", type=int, default=SAFE_X_MAX_COUNT)
    p.add_argument("--latent_dim_causal", type=int, default=LATENT_DIM_CAUSAL)
    p.add_argument("--latent_dim_recon", type=int, default=LATENT_DIM_RECON)
    p.add_argument("--hidden_dim_enc", type=int, default=HIDDEN_DIM_ENC)
    p.add_argument("--hidden_dim_head", type=int, default=HIDDEN_DIM_HEAD)
    p.add_argument("--num_lstm_layers", type=int, default=NUM_LSTM_LAYERS)
    p.add_argument("--max_epochs_joint", type=int, default=MAX_EPOCHS_JOINT)
    p.add_argument("--beta_kl", type=float, default=BETA_KL)
    p.add_argument("--lambda_orth", type=float, default=LAMBDA_ORTH)
    p.add_argument("--lambda_recon_final", type=float, default=LAMBDA_RECON_FINAL)
    p.add_argument("--phase1_ratio", type=float, default=PHASE1_RATIO)
    p.add_argument("--phase2_ratio", type=float, default=PHASE2_RATIO)
    p.add_argument("--uncertainty_clip_quantile", type=float, default=UNCERTAINTY_CLIP_QUANTILE)
...
def main():
    args = parse_args()
    global N_BOOTSTRAP, F_STAT_THRESHOLD, MIN_VALID_RESIDUALS
    global MIN_BOOTSTRAP_SUCCESS_RATE, SAFE_X_MAX_COUNT
    global LATENT_DIM_CAUSAL, LATENT_DIM_RECON, HIDDEN_DIM_ENC, HIDDEN_DIM_HEAD
    global NUM_LSTM_LAYERS, MAX_EPOCHS_JOINT, BETA_KL, LAMBDA_ORTH
    global LAMBDA_RECON_FINAL, PHASE1_RATIO, PHASE2_RATIO, UNCERTAINTY_CLIP_QUANTILE
    ...
    LATENT_DIM_CAUSAL = max(4, args.latent_dim_causal)
    LATENT_DIM_RECON = max(LATENT_DIM_CAUSAL + 4, args.latent_dim_recon)
    HIDDEN_DIM_ENC = max(16, args.hidden_dim_enc)
    HIDDEN_DIM_HEAD = max(8, args.hidden_dim_head)
    NUM_LSTM_LAYERS = max(1, args.num_lstm_layers)
    MAX_EPOCHS_JOINT = max(20, args.max_epochs_joint)
    BETA_KL = max(1e-4, args.beta_kl)
    LAMBDA_ORTH = max(0.0, args.lambda_orth)
    LAMBDA_RECON_FINAL = max(0.05, args.lambda_recon_final)
    PHASE1_RATIO = min(0.50, max(0.05, args.phase1_ratio))
    PHASE2_RATIO = min(0.50, max(0.05, args.phase2_ratio))
    if PHASE1_RATIO + PHASE2_RATIO > 0.85:
        PHASE2_RATIO = 0.85 - PHASE1_RATIO
    UNCERTAINTY_CLIP_QUANTILE = min(0.99, max(0.50, args.uncertainty_clip_quantile))
    ...
    print(f" 架构: LSTM-VAE 联合训练 + 微创新 A/B/C/D（调优版）")
    print(f" 微创新配置: {_fmt_innov_cfg(innov_cfg)}")
    print(f" 交叉拟合策略: {_fmt_cf_cfg(cf_cfg)}")
    print("=" * 70)
