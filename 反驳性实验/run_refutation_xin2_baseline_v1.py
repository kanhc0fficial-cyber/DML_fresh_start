"""
run_refutation_xin2_baseline_v1.py
====================================
XIN_2 因果推断反驳实验 — 基线方法 V1「随机森林 DML（RF-DML）」
...
用法：
  ...
  python run_refutation_xin2_baseline_v1.py --mode stability --sample_size 2000 --n_bootstrap 3

  # 全量稳定性诊断
  python run_refutation_xin2_baseline_v1.py --mode stability

  # 安慰剂反驳
  python run_refutation_xin2_baseline_v1.py --mode placebo --n_permutations 5 --workers 4

  # 随机混杂变量反驳
  python run_refutation_xin2_baseline_v1.py --mode random_confounder --workers 4

  # 数据子集反驳
  python run_refutation_xin2_baseline_v1.py --mode data_subset --workers 4

  # 全部实验一次跑
  python run_refutation_xin2_baseline_v1.py --mode all --workers 4
"""
...
# ── 实验结果输出目录（与 v3 共享目录，文件名含 _baseline_v1 后缀以区分）──
...
def _build_lag_features(X_norm: np.ndarray, n_lags: int) -> np.ndarray:
    ...
    T, d = X_norm.shape
    n_samples = T - n_lags
    # row_idx[i, k] = i + k → 每行取 X_norm[i:i+n_lags] 的行索引
...
def run_stability_diagnosis(df, ops, states, workers=4, dag_roles: dict = None):
    ...
    out_path = os.path.join(STABILITY_OUT_DIR, "stability_diagnosis_baseline_v1.csv")
...
def run_placebo(df, ops, states, n_permutations=5, workers=4, dag_roles: dict = None):
    ...
    if dag_roles is None:
        dag_roles = {}
    ckpt_path   = os.path.join(PLACEBO_OUT_DIR, "checkpoint_placebo_baseline_v1.jsonl")
    states_list = list(states)
    tasks = []
    for op in sorted(ops):
        if df[op].std() < 0.1:
    ...
    out_path = os.path.join(PLACEBO_OUT_DIR, "refutation_placebo_baseline_v1.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
...
def run_random_confounder(df, ops, states, n_confounders=5, n_repeats=1,
    ...
    if dag_roles is None:
        dag_roles = {}
    ckpt_path   = os.path.join(RANDOM_CONFOUNDER_OUT_DIR, "checkpoint_rc_baseline_v1.jsonl")
    states_list = list(states)
    print("[预计算原始 θ ...]")
    orig_thetas = {}
    for op in sorted(ops):
    ...
    out_path = os.path.join(RANDOM_CONFOUNDER_OUT_DIR, "refutation_rc_baseline_v1.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"结果已保存：{out_path}"); return df_out
...
def run_data_subset(df, ops, states, n_subsets=8, subset_frac=0.8,
    ...
    if dag_roles is None:
        dag_roles = {}
    ckpt_path   = os.path.join(DATA_SUBSET_OUT_DIR, "checkpoint_ds_baseline_v1.jsonl")
    states_list = list(states)
    ...
    out_path = os.path.join(DATA_SUBSET_OUT_DIR, "refutation_ds_baseline_v1.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"结果已保存：{out_path}"); return df_out
...
def parse_args():
    p = argparse.ArgumentParser(
        description="XIN_2 因果推断反驳实验 — 基线 V1（RF-DML）",
...
断点续传：检查点含 _baseline_v1 后缀，与 v3/v4/v5 不冲突。
        """,
    )
...
def main():
    ...
    print("=" * 70)
    print(f" XIN_2 因果推断反驳实验 — 基线 V1（RF-DML）")
