"""
run_refutation_xin2_baseline_v1_tuned.py
====================================
XIN_2 因果推断反驳实验 — 基线方法 V1 调优版「随机森林 DML（RF-DML）」

═══════════════════════════════════════════════════════════════════
  基线定位：提供可对比的传统机器学习基线，与创新版 v3/v4/v5（LSTM-VAE-DML）
  形成对照。本脚本不使用深度学习，仅用 sklearn 实现，确保能稳定运行。

  方法：随机森林 Double ML（RF-DML）
  ─────────────────────────────────────────────────────────────────
  核心思路（Chernozhukov et al. 2018 的 DML-PLM 框架）：
    ...
    2. 交叉拟合（Cross-Fitting）：
         - 在训练折用 RF 拟合 Ŷ = E[Y|X_lag] 和 D̂ = E[D|X_lag]
  ...
  为何选 RF 作 V1？
    - RF 是 DML 文献中最常用的 nuisance 估计器（EconML 默认后端）
    - 能处理高维、非线性、变量间交互
    - 对超参不敏感，运行稳定
    - 原生支持特征重要性（辅助解释变量选择）
...
用法：
  ...
  python run_refutation_xin2_baseline_v1_tuned.py --mode stability --sample_size 2000 --n_bootstrap 3

  # 全量稳定性诊断
  python run_refutation_xin2_baseline_v1_tuned.py --mode stability

  # 安慰剂反驳
  python run_refutation_xin2_baseline_v1_tuned.py --mode placebo --n_permutations 5 --workers 4

  # 随机混杂变量反驳
  python run_refutation_xin2_baseline_v1_tuned.py --mode random_confounder --workers 4

  # 数据子集反驳
  python run_refutation_xin2_baseline_v1_tuned.py --mode data_subset --workers 4

  # 全部实验一次跑
  python run_refutation_xin2_baseline_v1_tuned.py --mode all --workers 4
"""
...
# ── 实验结果输出目录（与 v3 共享目录，文件名含 _baseline_v1_tuned 后缀以区分）──
...
def refine_safe_x(op: str, df: pd.DataFrame, safe_x: list,
    ...
    """对 safe_x 做轻量裁剪，缓解高维小样本下的 RF 残差不稳定。"""
...
def train_one_op(op: str, df: pd.DataFrame, safe_x: list,
    ...
    对每个 bootstrap × fold：
      ...
      2. 在训练折用 RandomForestRegressor 拟合：
           Ŷ = E[Y | X_lag]   (head_Y)
           D̂ = E[D | X_lag]   (head_D)
...
# ═══════════════════════════════════════════════════════════════════
#  Worker 函数（与 v3 一致，调用本脚本的 train_one_op）
# ═══════════════════════════════════════════════════════════════════
...
def run_stability_diagnosis(df, ops, states, workers=4, dag_roles: dict = None):
    ...
    print(f" 架构：RF-DML 基线 V1（随机森林 nuisance + 滞后特征）")
    ...
    out_path = os.path.join(STABILITY_OUT_DIR, "stability_diagnosis_baseline_v1_tuned.csv")
    ...
    if not df_out.empty:
        ...
        if total > 0 and n_stable / total < 0.5:
            print("  [⚠] 超过一半操作变量不稳定，建议调整 RF 超参（max_depth, min_samples_leaf）")
        else:
...
def run_placebo(df, ops, states, n_permutations=5, workers=4, dag_roles: dict = None):
    ...
    if dag_roles is None:
        dag_roles = {}
    ckpt_path   = os.path.join(PLACEBO_OUT_DIR, "checkpoint_placebo_baseline_v1_tuned.jsonl")
    states_list = list(states)
    tasks = []
    for op in sorted(ops):
        if df[op].std() < 0.1:
    ...
    out_path = os.path.join(PLACEBO_OUT_DIR, "refutation_placebo_baseline_v1_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
...
def run_random_confounder(df, ops, states, n_confounders=5, n_repeats=1,
    ...
    if dag_roles is None:
        dag_roles = {}
    ckpt_path   = os.path.join(RANDOM_CONFOUNDER_OUT_DIR, "checkpoint_rc_baseline_v1_tuned.jsonl")
    states_list = list(states)
    print("[预计算原始 θ ...]")
    orig_thetas = {}
    for op in sorted(ops):
    ...
    out_path = os.path.join(RANDOM_CONFOUNDER_OUT_DIR, "refutation_rc_baseline_v1_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"结果已保存：{out_path}"); return df_out
...
def run_data_subset(df, ops, states, n_subsets=8, subset_frac=0.8,
    ...
    if dag_roles is None:
        dag_roles = {}
    ckpt_path   = os.path.join(DATA_SUBSET_OUT_DIR, "checkpoint_ds_baseline_v1_tuned.jsonl")
    states_list = list(states)
    ...
    out_path = os.path.join(DATA_SUBSET_OUT_DIR, "refutation_ds_baseline_v1_tuned.csv")
    df_out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"结果已保存：{out_path}"); return df_out
...
def parse_args():
    p = argparse.ArgumentParser(
        description="XIN_2 因果推断反驳实验 — 基线 V1 调优版（RF-DML）",
...
断点续传：检查点含 _baseline_v1_tuned 后缀，与原始脚本不冲突。
        """,
    )
    ...
    p.add_argument("--f_stat_threshold", type=float, default=F_STAT_THRESHOLD,
                   help=f"弱工具过滤阈值（默认 {F_STAT_THRESHOLD}）")
    p.add_argument("--min_valid_residuals", type=int, default=MIN_VALID_RESIDUALS,
                   help=f"最少有效残差数（默认 {MIN_VALID_RESIDUALS}）")
    p.add_argument("--min_success_rate", type=float, default=MIN_BOOTSTRAP_SUCCESS_RATE,
                   help=f"Bootstrap 最低成功率（默认 {MIN_BOOTSTRAP_SUCCESS_RATE:.2f}）")
    p.add_argument("--max_safe_x", type=int, default=SAFE_X_MAX_COUNT,
                   help=f"safe_x 上限（默认 {SAFE_X_MAX_COUNT}，0=不裁剪）")
    p.add_argument("--rf_n_estimators", type=int, default=RF_N_ESTIMATORS)
    p.add_argument("--rf_max_depth", type=int, default=RF_MAX_DEPTH)
    p.add_argument("--rf_min_samples_leaf", type=int, default=RF_MIN_SAMPLES_LEAF)
    p.add_argument("--rf_max_features", type=str, default=str(RF_MAX_FEATURES),
                   help="RF max_features，支持 sqrt/log2 或 0~1 浮点比例")
...
def main():
    ...
    global RF_N_ESTIMATORS, RF_MAX_DEPTH, RF_MIN_SAMPLES_LEAF, RF_MAX_FEATURES
    ...
    RF_MIN_SAMPLES_LEAF = max(1, args.rf_min_samples_leaf)
    rf_max_features_arg = args.rf_max_features.strip().lower()
    if rf_max_features_arg in {"sqrt", "log2"}:
        RF_MAX_FEATURES = rf_max_features_arg
    else:
        RF_MAX_FEATURES = float(rf_max_features_arg)

    print("=" * 70)
    print(f" XIN_2 因果推断反驳实验 — 基线 V1 调优版（RF-DML）")
    print(f" 模式: {args.mode.upper()}  |  并行线程: {args.workers}  |  Bootstrap: {N_BOOTSTRAP}")
    print(f" 架构: Random Forest DML + 滞后特征（SEQ_LEN={SEQ_LEN}）")
    ...
    print(f"   调优阈值：F≥{F_STAT_THRESHOLD}, 残差≥{MIN_VALID_RESIDUALS}, "
