"""
run_joint_causal_xin2_fixed.py
=================================
专为 XIN_2 管线打造的【联合优化生成式因果管线】 (End-to-End VAE-LSTM DML)
...
路径适配说明（2026-04 重构）：
  ...
  - 数据范围收窄为 Group B（XIN2 专线）+ Group C（公用设备）
  - 暂时关闭双生产线处理，仅运行 XIN2 管线
"""
...
OUT_DIR = r"C:\DML_fresh_start\双重机器学习\结果\joint_causal_xin2"
...
def build_xin2_data():
    """
    读取 X 特征矩阵与 XIN2 目标变量，按以下规则筛选列：
      - 变量必须属于 Group B（XIN2 专线）或 Group C（公用设备）
    ...
    print(f"[数据准备] Group B+C 共 {len(all_bc_vars)} 个变量，其中 operable={len(operable_set)}，observable={len(observable_set)}")
...
def main():
    print("=" * 70)
    print(" 启动 XIN_2 端到端联合优化因果推断 (已修复全部已知问题)")
    print("=" * 70)

    # [ADAPT] build_xin2_data() 现在同时返回操作集和可观测集
    ...
    out_path = os.path.join(OUT_DIR, "joint_causal_dml_xin2_fixed.csv")
